...
me_ok <- rlang::is_installed(c("marginaleffects (>= 0.25.1)", "sandwich")) &&
  !isTRUE(as.logical(Sys.getenv("NOT_CRAN", "false")))
su_ok <- rlang::is_installed("survival")
boot_ok <- rlang::is_installed("boot")
```
...
Other packages may be of use but are not used here. There are alternatives to the *marginaleffects* package for computing average marginal effects, including *margins* and *stdReg2*. The *survey* package can be used to estimate robust SEs incorporating weights and provides functions for survey-weighted generalized linear models and Cox-proportional hazards models.
